...
class MongoDao:
    ...
    """
    def find_all(self, collection, filter = {}, projection= {}):
        collection_conn = self.__database.get_collection(collection)
        return list(collection_conn.find(filter, projection))
    """
    
    def find_all(self, collection, filter={}, projection={}):
        collection_conn = self.__database.get_collection(collection)
        cursor = collection_conn.find(filter, projection)
        documents = []
        for doc in cursor:
            doc['_id'] = str(doc['_id'])  # Convertir solo el _id
